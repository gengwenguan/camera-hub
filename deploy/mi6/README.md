...
由 `AssetManager` 完成下载、SHA-256 校验、原子替换和 worker 重新初始化。Cargo
构建期所需的 sherpa 共享库由 `sherpa-onnx-sys` 获取，安装器只复制构建产物。
...
```text
ac-on         RN02S 方案 D：制冷 26°C、自动风、ECO
ac-on-no-eco  RN02S 方案 C：制冷 26°C、自动风
ac-off        RN02S 关机方案 B
```
